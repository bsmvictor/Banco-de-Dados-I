...
# COMANDO SQL PARA BUSCA DE DADOS

#OS CAMPOS QUE GOSTARIAMOS DE BUSCAR
SELECT Dnumero, Dnome
# A(S) TABELA(S) QUE SE ENCONTRA(M) ESTES CAMPOS
FROM Departamento
# FILTRAR A INFORMAÇÃO QUE DESEJA VIASUALIZAR
WHERE Dnumero >= 4;

#----------------------------------------------------------------------------------------------------------
# FAZENDO UM JOIN (OU JUNÇÃO), JOINS SO PODEM SER FEITOS COM TABELAS QUE SE RELACIONAM

# o join é um select com mais de uma tabela seguido de um condicional
#(FK de uma tabela = PK da outra tabela)
SELECT Pnome, Dnome
...
# CRIANDO UM JOIN COM 3 TABELAS
SELECT e.PNome, p.PNome, te.Horas
